const string Input = @"zihrtxagncfpbsnolxydujjmqv
...
var ids = Input.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);

var counts = ids.Select(_ => _.ToCharArray().GroupBy(_ => _).ToDictionary(_ => _.Key, _ => _.Count())).ToList();

Console.WriteLine("Part 1: {0}", counts.Count(_ => _.Any(kv => kv.Value == 2)) * counts.Count(_ => _.Any(kv => kv.Value == 3)));
...
for (int i = 0; i < ids.Length; i++)
{
    for (int j = i + 1; j < ids.Length; j++)
    {
        ...
        for (int k = 0; k < ids[i].Length; k++)
        {
            if (ids[i][k] == ids[j][k])
            {
                commonLetters += ids[i][k];
            }
        }

        if (commonLetters.Length + 1 == ids[i].Length)
        {
            Console.WriteLine("Part 2: {0}", commonLetters);
            return;
        }
    }
}