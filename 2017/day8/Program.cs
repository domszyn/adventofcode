const string Input = @"o dec -427 if wnh < -1
...
var registerValue = (string name) =>
{
    if (!registers.ContainsKey(name)) { registers[name] = 0; }
    return registers[name];
};

var less = (string register, int value) => registerValue(register) < value;
var lessOrEqual = (string register, int value) => registerValue(register) <= value;
var greater = (string register, int value) => registerValue(register) > value;
var greaterOrEqual = (string register, int value) => registerValue(register) >= value;
var equal = (string register, int value) => registerValue(register) == value;
var notEqual = (string register, int value) => registerValue(register) != value;
var conditionMet = (string register, string op, int value) =>
{
    var conditionCheck = (string r, int v) => false;
    switch (op)
    {
        case "<": conditionCheck = less; break;
        case "<=": conditionCheck = lessOrEqual; break;
        case ">": conditionCheck = greater; break;
        case ">=": conditionCheck = greaterOrEqual; break;
        case "==": conditionCheck = equal; break;
        case "!=": conditionCheck = notEqual; break;
    }

    return conditionCheck(register, value);
};
...
foreach (var instruction in instructions)
{
    ...
    var r = words[0];
    var v = Convert.ToInt32(words[2]);
    if (words[1] == "dec") { v *= -1; }

    if (conditionMet(words[4], words[5], Convert.ToInt32(words[6])))
    {
        var rv = registerValue(r);
        if (rv > part2) { part2 = rv; }
        if (rv + v > part2) { part2 = rv + v; }
        registers[r] = rv + v;
    }
}

Console.WriteLine("Part 1: {0}, Part 2: {1}", registers.Values.Max(), part2);
