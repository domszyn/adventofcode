const string Input = @"gbyvdfh (155) -> xqmnq, iyoqt, dimle
oweiea (97)
szhxrs (14)
pjvwsiw (23)
...
var parseTower = (string s) =>
{
    ...
    tower.OwnWeight = Convert.ToInt32(nameParts[1].Substring(1, nameParts[1].Length - 2));
    if (parts.Length == 2)
    {
...
Input.Split("\r\n").Select(parseTower).ToList();

var diff = 0;
Tower bottomTower = null;
foreach (Tower t in towers.Values)
{
    if (!towers.Values.SelectMany(t => t.Disc.Select(_ => _.Name)).Contains(t.Name)) {
        Console.WriteLine("Part 1: {0}", t.Name);
        bottomTower = t;
        var unbalanced = bottomTower.UnbalancedSubTower();
        diff = unbalanced.TotalWeight - bottomTower.Disc.First(_ => _.Name != unbalanced.Name).TotalWeight;
        break;
    }
}
...
do {
    ...
    if (unbalanced == null) {
        Console.WriteLine("Part 2: {0}", bottomTower.OwnWeight - diff);
        break;
    } else {
        bottomTower = unbalanced;
    }
} while(true);
