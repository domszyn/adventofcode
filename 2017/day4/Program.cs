const string Input = @"bdwdjjo avricm cjbmj ran lmfsom ivsof
...
var distinctWordsPasswords = Input.Split("\r\n").Select(_ => _.Split(" "))
    .Where(words => words.Count() == words.Distinct().Count())
    .ToList();
Console.WriteLine("Part 1: {0}", distinctWordsPasswords.Count);

Console.WriteLine("Part 2: {0}", distinctWordsPasswords
    .Where(words => words.All(
        word => words.Count(
            _ => _.ToCharArray().OrderBy(c => c).SequenceEqual(word.ToCharArray().OrderBy(c => c))) == 1))
    .Count());
